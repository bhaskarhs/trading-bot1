...
<p>Nifty LTP vs open (and gap vs previous close) maps to STRONG_UP / MILD_UP / FLAT / DOWN. Then LTP batches of 25 over the session universe (Nifty 500 by default, ~120 if <code>UNIVERSE_MODE=bundled</code>) keep names that moved ≥1% in the “right” direction (flat/up = gainers only), plus anything already held. RSI still runs on at most 50 names. BUYs need a bounce on the last bar; flat/up days also need that +1% green. Fallback: first 50 names if nothing qualifies.</p>
